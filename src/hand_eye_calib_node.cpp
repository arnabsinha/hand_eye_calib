...
class hand_eye_calib{
	private:
        ...
        string kinect_frame, robot_base_frame;
#if 0
    ...
    bool belongs(int r, int c, vector<Point2f> points, int &pos){
    }
        
  void createCloud(const cv::Mat &depth, const cv::Mat &color, pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud, vector<Point2f> pointBuf, PointCloud &actCloud) const
  {
    const float badPoint = std::numeric_limits<float>::quiet_NaN();
    actCloud.points.clear();
    for(int i=0;i<pointBuf.size();i++){
        pcl::PointXYZ pt;
        actCloud.points.push_back(pt);
    }
...
//          itP->rgba = 0;
        ...
        bool f = false;
        for(int i=0;i<pointBuf.size();i++)
            if(r==(int)pointBuf[i].y && c==(int)pointBuf[i].x){
                pos = i;
                f = true;
            }
        if(f){
            actCloud.points[pos].x = itP->x;
            actCloud.points[pos].y = itP->y;
            actCloud.points[pos].z = itP->z;
            cout<<pos<<" "<<r<<" "<<c<<" "<<depthValue<<" "<<*itX<<" "<<y<<"\t"<<itP->x<<" "<<itP->y<<" "<<itP->z<<endl;
        }
#if 0
        itP->b = itC->val[0];
        itP->g = itC->val[1];
  ...
  {
    const float fx = 1.0f / cameraM.at<double>(0, 0);
    const float fy = 1.0f / cameraM.at<double>(1, 1);
    const float cx = cameraM.at<double>(0, 2);
    const float cy = cameraM.at<double>(1, 2);
    float *it;
...
#if 0    
    this->color.cols = 1920;
    this->color.rows = 1080;
    this->cloud = pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>());
    this->cloud->header.frame_id =  "kinect2_rgb_optical_frame";
    this->cloud->height = this->color.rows;
    this->cloud->width = this->color.cols;
    this->cloud->is_dense = false;
    this->cloud->points.resize(this->cloud->height * this->cloud->width);
...
void hand_eye_calib::callback(const ImageConstPtr& image, const ImageConstPtr& depth)
{
  // Solve all of perception here...
    ...
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), kinect_frame, robot_base_frame));
...
//        this->color = cv_ptr->image.clone();
//        this->depth = depth_ptr->image.clone();
...
//            drawChessboardCorners( view, boardSize, Mat(pointBuf), found );
...
#if 1
            int winSz = 1;
            double sc = 1.0/(double)(winSz*winSz);
            for(size_t i=0;i<pointBuf.size();i++){
                ...
                pt.z = (static_cast<double>(depth_ptr->image.at<uint16_t>(pointBuf[i])))/1000.0;
                transform_polar_to_cart(pt,cpt);
//                cout<<i<<" "<<pt.x<<" "<<pt.y<<" "<<pt.z<<"\t"<<cpt.x<<" "<<cpt.y<<" "<<cpt.z<<endl;
                image_points_.push_back(cpt);
            }
#endif

            image_points_.header.frame_id = kinect_frame;

//            createCloud(this->depth,this->color,cloud,pointBuf,image_points_);
            pub_image_pcl.publish(image_points_);
//            pub_image_pcl.publish(image_points_);
            getPhyPclFromTF(physical_points_);
            physical_points_.header.frame_id = robot_base_frame;
...
void hand_eye_calib::getPhyPclFromTF(PointCloud &physical_points_){
    ...
    try{
        listener.lookupTransform(robot_base_frame,  "end_effector", ros::Time(0), tr);
        //ROS_INFO_STREAM(tr.getOrigin().x());
    }
    catch (tf::TransformException ex){
      ROS_ERROR("%s",ex.what());
...
//    pcl::registration::TransformationEstimationLM < pcl::PointXYZ, pcl::PointXYZ > svd_estimator;
    ...
    for(int i=0;i<pos.size();i++){
        pcl::PointXYZ pt;
        pt.x = image_points_.points[pos[i]].x;
        pt.y = image_points_.points[pos[i]].y;
        pt.z = image_points_.points[pos[i]].z;
...
#if 1
    fx = 1.0f / cameraM.at<double>(0, 0);
    fy = 1.0f / cameraM.at<double>(1, 1);
    pto.z = pti.z;
